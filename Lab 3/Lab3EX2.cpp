...
int main()
{
	...
	while (true)
	{
		...
		// Start  - immediately stop the Kobuki's movement = 7
		// Select - exit the script and close the Kobuki's connection cleanly = 6
		if (joystick.sample(&event))
		{
			...
			if (event.isAxis())
			{
				...
				else if (event.number == 6 && event.value == -32767)
				{
					// move left
					movement(120, 1);
				}
				else if (event.number == 7 && event.value == -32767)
				{
					// move up
					movement(radius, 0);
				}
